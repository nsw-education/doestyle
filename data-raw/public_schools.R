...
# Metadata was extracted from https://www.metadata.nsw.gov.au/item/9544/distribution/nsw-public-schools-master-dataset
# via the Aristotle API

library(dplyr)
library(labelled)
library(readr)

metadata <- read_csv("data-raw/data_dictionary.csv",
                     col_types = cols(
                       logical_path = col_character(),
                       data_element_id = col_double(),
                       concept_id = col_double(),
                       property_id = col_double(),
                       property_name = col_character()
                     ))

public_schools_unlabelled <- read_csv(
...
# Unfortunately, we don't have metadata for every column
data_dictionary <- tibble(name = colnames(public_schools_unlabelled)) |>
  left_join(metadata, join_by(name == logical_path))
variable_labels <- setNames(data_dictionary$property_name, data_dictionary$name)

public_schools <- set_variable_labels(public_schools_unlabelled,
